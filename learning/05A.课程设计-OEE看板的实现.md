...
│   │    ├── App.css           # 样式文件
│   │    └── index.js          # 入口文件
│   ├── package.json            # React项目配置及依赖（含 react-gauge-chart）
│   └── vite.config.js 或其他   # 如用Vite（或 CRA 则为无）
│
└── README.md                  # 项目说明（可选）
```
...
const App = () => {
  ...
  const calculateOEE = (records) => {
    const recent = records.slice(-60); // 最近5分钟数据（5秒一条）
    ...
    const speed = 120; // 标准速度 (假设120件/小时)
    const performance = (outputDelta / (runTime / 3600)) / speed;

    const goodOutputDelta = recent[recent.length - 1].good_output - recent[0].good_output;
  ...
  return (
    <div className="App">
      ...
      <div className="gauges">
        <GaugeChart id="avail" nrOfLevels={20} percent={metrics.availability} textFormat="percent" />
        <GaugeChart id="perf" nrOfLevels={20} percent={metrics.performance} textFormat="percent" />
        <GaugeChart id="qual" nrOfLevels={20} percent={metrics.quality} textFormat="percent" />
      </div>
