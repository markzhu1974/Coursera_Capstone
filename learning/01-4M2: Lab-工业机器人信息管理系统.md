...
## 3. 新建CSV导入工具 (import_maintenance.py)
...
导入的维修记录样例文件在code/01-4_Lab/M2/mro_record_all.csv。把这个文件拷贝到本项目当前目录中。

然后，在终端执行： 

```
python database.py // 创建表，需要事先执行。 在server.py执行时也会调用它来创建表 
python import_maintenance.py mro_record_all.csv 
```
数据库建好并且导入数据后，在VS Code中，如果安装了 SQLite3 扩展，就可以点击robots.db文件打开数据库查看导入的robot数据和maintenance记录数据

## 4. 前端页面

### 修改设备管理页面 (static/index.html)
...
### 新建维修记录管理页面 (static/maintenance.html)
...
### 修改设备页面脚本JavaScript (static/script.js)
...
### 新建维修记录页面脚本JavaScript (static/maintenance.js)
...
代码参考 [code/01-4_Lab/M2](code/01-4_Lab/M2)